...
def agent_tool_router(state: GlobalState):
...
graph_builder.add_node("search_result_aggregator", search_result_aggregator)
...
graph_builder.add_edge("tools", "search_result_aggregator")
graph_builder.add_edge("search_result_aggregator", "agent")

graph = wrap_with_scoped_filler(
    graph_builder.compile().with_config({"configurable": singleton_config()})
)
