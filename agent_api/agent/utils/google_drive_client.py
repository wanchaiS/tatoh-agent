...
def list_images_in_folder(path: str) -> List[str]:
    ...
    if (
        not folder_info
        or folder_info["mimeType"] != "application/vnd.google-apps.folder"
    ):
        return []
...
def _read_sheet(spreadsheet_id: str, range_name: str) -> List[List]:
    ...
    try:
        ...
        raise RuntimeError(
            f"Unexpected error reading sheet {spreadsheet_id}: {e}"
        ) from e
...
def _find_file_by_path(path: str) -> Optional[Dict]:
    ...
    try:
        ...
        raise RuntimeError(
            f"Unexpected error searching for GDrive path {path}: {e}"
        ) from e
    return None
