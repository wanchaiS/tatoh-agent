...
def find_boat_schedules(req_origin: Location, req_destination: Location) -> List[Dict]:
    ...
    for schedule in all_schedules:
        ...
        if (
            schedule_from
            and schedule_to
            and (req_origin in schedule_from or schedule_from in req_origin)
            and (req_destination in schedule_to or schedule_to in req_destination)
        ):
            # format values
            ...
            schedule["young_children_price"] = _parse_price(
                schedule.get("young_children_price", 0)
            )
