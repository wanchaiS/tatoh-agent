...
async def list_rooms(db: AsyncSession = Depends(get_db)) -> list[RoomModel]:
    return await RoomManagementService(db).list_rooms()


@router.get("/{id}", response_model=RoomResponse)
async def get_room(id: int, db: AsyncSession = Depends(get_db)) -> RoomModel:
    return await RoomManagementService(db).get_room(id)


@router.post("", response_model=RoomResponse, status_code=status.HTTP_201_CREATED)
async def create_room(
    data: RoomCreate, db: AsyncSession = Depends(get_db)
) -> RoomModel:
...
async def update_room(
    id: int, data: RoomUpdate, db: AsyncSession = Depends(get_db)
) -> RoomModel:
    return await RoomManagementService(db).update_room(id, data)


@router.delete("/{id}", status_code=status.HTTP_204_NO_CONTENT)
async def delete_room(id: int, db: AsyncSession = Depends(get_db)) -> None:
