...
class BoatScheduleRepository:
    ...
    async def get_distinct_locations(self) -> list[str]:
        origins = await self.db.execute(select(BoatSchedule.origin).distinct())
...
class BusScheduleRepository:
    ...
    async def get_distinct_locations(self) -> list[str]:
        origins = await self.db.execute(select(BusSchedule.origin).distinct())
        destinations = await self.db.execute(select(BusSchedule.destination).distinct())
        all_locs = set(origins.scalars().all()) | set(destinations.scalars().all())
        return sorted(all_locs)
