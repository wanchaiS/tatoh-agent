...
RESORT_ROOMS = ["s1","s2","s3","s4","s5","s6","s7","s8","s9","s10","s11","s12", "s14", "v1", "v2", "v3"]
...
def _validate(
    ...
    if total_guests is not None and (total_guests < 1 or total_guests > 30):
        errors.append(f"Guest count ({total_guests}) should be at least 1 and likely less than 30.")
    ...
    if requested_rooms is not None:
        for room in requested_rooms:
            if room.lower() not in RESORT_ROOMS:
                errors.append(f"Room {room} is not a valid room type. Valid room types are: {', '.join([r.upper() for r in RESORT_ROOMS])}.")
...
def update_criteria(
    ...
    effective_duration = duration_nights if duration_nights is not None else current.duration_nights
    effective_guests = total_guests if total_guests is not None else current.total_guests
    effective_windows = incoming_windows if incoming_windows else current.date_windows
    effective_requested_rooms = requested_rooms if requested_rooms is not None else current.requested_rooms

    # ── Validate ──────────────────────────────────────────────────────
    errors = _validate(effective_windows, effective_duration, effective_guests, effective_requested_rooms)
