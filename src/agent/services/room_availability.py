from typing import Any, List, Tuple, Dict, Set, TypedDict
...
class RoomAvailabilityService:
    ...
    def get_availability(self, search_start: datetime, search_end: datetime) -> Dict[str, RoomAvailabilityData]:
        """
        ...
        while current_date < search_end:
            ...
            if not covered:
                ...
                pms_data = fetch_room_availability_window(current_date.strftime("%Y-%m-%d"))
                pms_start = datetime.strptime(pms_data["from"], "%Y-%m-%d")
                pms_end = datetime.strptime(pms_data["to"], "%Y-%m-%d") + timedelta(days=1)
                ...
                for room_no, room_info in pms_data["rooms"].items():
                    if room_no not in self.rooms_availability:
                        ...
                        self.rooms_availability[room_no]["dates"].update(room_info["dates"])
        ...
        valid_dates = {(search_start + timedelta(days=i)).strftime('%Y-%m-%d') for i in range((search_end - search_start).days)}
        result_rooms = {}
